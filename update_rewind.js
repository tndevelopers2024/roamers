...
// Use more forgiving regexes
const cssPattern = /(\s*)\.rewind-section\s*\{\s*padding:.*?\.rewind-card img\s*\{\s*width: 100%;\s*height: 100%;\s*object-fit: cover;\s*transition: transform 0\.5s ease;\s*\}/gs;
...
const jsPattern = /(\s*)new Swiper\('\.rewind-swiper', \{.*?320:\s*\{.*?\},.*?1024:\s*\{.*?\}\s*\} \}\);/gs;
const jsPatternAlternative = /(\s*)const rewindSwiper = new Swiper\('\.rewind-swiper'.*?\}\);\s*$/gms;
const jsPatternOriginal = /(\s*)new Swiper\('\.rewind-swiper', \{.*?\}\);/gs;
...
files.forEach(filename => {
    ...
    // Replace JS
    content = content.replace(jsPatternOriginal, (match, indent) => {
