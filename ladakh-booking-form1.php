...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $mode         = $_POST['mode']; // <- MISSING in original PHP
    $pickup       = $_POST['pickup'];
    ...
    try {
        ...
        $mail->Password = $smtp_configs['default']['pass']; // Use Gmail app password
        $mail->SMTPSecure = $smtp_configs['default']['secure'];
        $mail->Port = $smtp_configs['default']['port'];

        // Recipients
        $mail->setFrom('info@roamers.in', 'Booking Form');
        $mail->addAddress('info@roamers.in'); // You can add another recipient if needed

        // Attachments (optional image)
        $mail->addEmbeddedImage('assets/img/ladakh/ladakh-mail.jpeg', 'ladakhImage');

        // Content
        $mail->isHTML(true);
        $mail->Subject = 'New Leh Ladakh with Umling la Booking Submission';

        $mail->Body = "
            <h2 style='color: #4ec0db; text-align: center;'>Leh Ladakh with Umling la - $date</h2>
            <center>
                <img src='cid:ladakhImage' style='width:400px; height:auto; margin-bottom:20px;' />
            </center>
            <table border='1' cellpadding='8' cellspacing='0' style='border-collapse: collapse;'>
                ...
                <tr><th align='left'>Mode of Travel </th><td>$mode</td></tr>
        ...
        $mail->send();
        echo "<script>alert('Booking successful! We will contact you soon.'); window.location.href = 'ladakh-booking-form.php';</script>";
    } catch (Exception $e) {
